...
public:
	...
	int do_some_test(libop* op) {
		...
		ver = m_op->Ver();
		std::wcout << L"ver:" << ver << std::endl;

		m_op->GetBasePath(str);
		std::wcout << L"GetBasePath:" << str << std::endl;
		...
		wstring colors[3];
		m_op->GetColor(5, 5, colors[0]);
		m_op->GetColor(10, 10, colors[1]);
		m_op->GetColor(20, 20, colors[2]);
		wstring offcolor = L"5|5|" + colors[1] + L"-050505,15|15|" + colors[2] + L"-050505";
		for (int dir = 0; dir < 4; dir++) {
