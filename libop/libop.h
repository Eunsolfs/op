...
#pragma region 算法
	//A星算法
	void AStarFindPath(long mapWidth, long mapHeight, const wchar_t* disable_points,
					   long beginX, long beginY, long endX, long endY, std::wstring& ret);
...
#pragma region windows api
	...
	void EnumWindowByProcess(const wchar_t* process_name, const wchar_t* title,
							 const wchar_t* class_name, long filter, std::wstring& ret);
	...
	//根据指定的进程名字，来查找可见窗口
	void FindWindowByProcess(const wchar_t* process_name, const wchar_t* class_name,
...
#pragma region image
	...
	//查找指定区域内的图片
	void FindPic(long x1, long y1, long x2, long y2, const wchar_t* files, const wchar_t* delta_color,
				 double sim, long dir, long* x, long* y, long* ret);

	//查找多个图片
	void FindPicEx(long x1, long y1, long x2, long y2, const wchar_t* files, const wchar_t* delta_color,
				   double sim, long dir, std::wstring& retstr);

	//这个函数可以查找多个图片, 并且返回所有找到的图像的坐标.此函数同FindPicEx.只是返回值不同.(file1,x,y|file2,x,y|...)
	void FindPicExS(long x1, long y1, long x2, long y2, const wchar_t* files, const wchar_t* delta_color,
					double sim, long dir, std::wstring& retstr);
	...
	void FindColorBlockEx(long x1, long y1, long x2, long y2, const wchar_t* color,
...
public:
	...
	//延时指定范围内随机毫秒,过程中不阻塞UI操作
	bool waits(long mis_min, long mis_max);

#pragma region Background
	bool BindWindow(long hwnd, const wchar_t* display, const wchar_t* mouse,
...
#pragma region mouse & keyboard
	...
	//鼠标相对于上次的位置移动rx,ry.
	bool MoveR(long x, long y);

	//把鼠标移动到目的点(x,y)
	bool MoveTo(long x, long y);

	//把鼠标移动到目的范围内的任意一点
	bool MoveToEx(long x, long y, long w, long h);
	...
	//设置鼠标单击或者双击时,鼠标按下和弹起的时间间隔
	bool SetMouseDelay(const wchar_t* type, long delay);

	//获取指定的按键状态.(前台信息,不是后台)
	bool GetKeyState(long vk_code);

	//按住指定的虚拟键码
	bool KeyDown(long vk_code);

	//按住指定的虚拟键码
	bool KeyDownChar(const wchar_t* vk_code);

	//弹起来虚拟键vk_code
	bool KeyUp(long vk_code);

	//弹起来虚拟键vk_code
	bool KeyUpChar(const wchar_t* vk_code);

	//等待指定的按键按下 (前台,不是后台)
	bool WaitKey(long vk_code, long time_out);

	//弹起来虚拟键vk_code
	bool KeyPress(long vk_code);

	bool KeyPressChar(const wchar_t* vk_code);
	//设置按键时,键盘按下和弹起的时间间隔

	bool SetKeypadDelay(const wchar_t* type, long delay);
	//根据指定的字符串序列，依次按顺序按下其中的字符

	bool KeyPressStr(const wchar_t* key_str, long delay);
#pragma endregion

#pragma region image
	//抓取指定区域(x1, y1, x2, y2)的图像, 保存为file
	bool Capture(long x1, long y1, long x2, long y2, const wchar_t* file_name);

	//比较指定坐标点(x,y)的颜色 true=颜色一样
	bool CmpColor(long x, long y, const wchar_t* color, double sim);

	//查找指定区域内的颜色 
	bool FindColor(long x1, long y1, long x2, long y2, const wchar_t* color,
				   double sim, long dir, long* x, long* y);

	//根据指定的多点查找颜色坐标
	bool FindMultiColor(long x1, long y1, long x2, long y2,
						const wchar_t* first_color, const wchar_t* offset_color,
						double sim, long dir, long* x, long* y);

	//查找指定区域内的图片
	bool FindPic(long x1, long y1, long x2, long y2, const wchar_t* files, const wchar_t* delta_color,
				 double sim, long dir, long* x, long* y);

	//获取(x,y)的颜色
	bool GetColor(long x, long y, std::wstring& ret);

	bool SetDisplayInput(const wchar_t* mode);

	bool LoadPic(const wchar_t* file_name);

	bool FreePic(const wchar_t* file_name);

	//从内存加载要查找的图片
	bool LoadMemPic(const wchar_t* file_name, void* data, long size);

	bool GetScreenData(long x1, long y1, long x2, long y2, size_t* data);

	bool GetScreenDataBmp(long x1, long y1, long x2, long y2, size_t* data, long* size);
#pragma endregion
